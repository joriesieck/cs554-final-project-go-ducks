...
export default function CreateUser() {
  ...
  const createUser = async (e) => {
    ...
    if (username && typeof username !== 'string')
      errorList.push('Username must be a string.');
    if (email && typeof email !== 'string')
      errorList.push('Email must be a string.');
    if (password && typeof password !== 'string')
      errorList.push('Password must be a string.');
    // trim and make sure all values are nonempty
    username = username.trim();
    email = email.trim();
    // don't trim password
    if (username === '')
      errorList.push('Username must contain at least one character.');
    if (email === '')
      errorList.push('Email must contain at least one character.');
    if (password === '')
      errorList.push('Password must contain at least one character.');
  ...
  return (
    <div id="create-user">
      ...
      {errors && (
        <Alert severity="error" id="create-user-errors">
          <ul>
            {errors.map((error) => (
