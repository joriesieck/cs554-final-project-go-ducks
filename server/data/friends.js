...
async function getPendingFriendByName(username, friendName){
    ...
    const inPending = user.pending_friends.findIndex( e => e.pendingId.equals(pending._id))
    if (inPending === -1){
...
async function addFriend(requesterName, requesteeName){
    ...
    const inPending = requester.pending_friends.findIndex((e) => e.pendingId.equals(requestee._id));
    ...
    const requesteeUpdate = await userCollection.updateOne(
        { _id: requestee._id },
        { $addToSet: { pending_friends: { pendingId: requester._id, status: 'received'}}}
    )
    ...
    // return updated requester and requestee (will update redis cache w these)
...
async function acceptFriend(username, pendingName){
    ...
    const inPending = user.pending_friends.some((e) => e.pendingId.equals(pending._id) && e.status === 'received');
